...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        pattern = os.path.join(commands_dir, 'shorten_*.py')
        ...
        for fetch_file in fetch_files:
            ...
            try:
                ...
                shorten_command_class = getattr(module, 'Command', None)
                if shorten_command_class is None:
                    self.stdout.write(f"Module '{module_name}' に Command クラスが見つかりません。")
                    continue

                # Commandクラスのインスタンスを作成
                shorten_command = shorten_command_class()
                
                # ハンドルメソッドを実行
                self.stdout.write(f"=== Fetching {module_name} ===")
                shorten_command.handle()
